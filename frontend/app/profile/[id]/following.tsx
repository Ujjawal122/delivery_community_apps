...
import { getFollowing } from '../../api/client';
...
export default function FollowingScreen() {
  ...
  useEffect(() => {
    ...
    const fetchFollowing = async () => {
      try {
        const res = await getFollowing(id, 50, 0);
  ...
  return (
    <View style={[styles.root, { backgroundColor: theme.bg }]}>
      <View style={[styles.header, { backgroundColor: theme.bgCard, borderBottomColor: theme.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: theme.textPrimary }]}>Following</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.centered}>
              <Text style={{ color: theme.textMuted }}>Not following anyone yet.</Text>
            </View>
          }
        />
      )}
